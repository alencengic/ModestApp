import { DateTime } from "luxon";
import { openDatabase } from "./db_connection";

type JournalEntryRow = { id: number; content: string; date: string };

export const insertJournalEntry = async (note: string, date: string) => {
  const db = await openDatabase();

  await db.execAsync(`
    CREATE TABLE IF NOT EXISTS journal_entries (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      note TEXT,
      date TEXT
    );
  `);

  return await db.runAsync(
    "INSERT INTO journal_entries (note, date) VALUES (?, ?)",
    note,
    date
  );
};

export const getJournalEntriesByRange = async (
  fromDate: Date,
  toDate: Date
): Promise<{ id: number; content: string; date: string }[]> => {
  const db = await openDatabase();

  const fromISO = DateTime.fromJSDate(fromDate).startOf("day").toISO();
  const toISO = DateTime.fromJSDate(toDate).endOf("day").toISO();

  const rows = await db.getAllAsync(
    `SELECT id, note as content, date FROM journal_entries WHERE date BETWEEN ? AND ? ORDER BY date DESC`,
    fromISO,
    toISO
  );

  return (rows as JournalEntryRow[]).map((row) => ({
    id: row.id,
    content: row.content,
    date: row.date,
  }));
};

export const getJournalEntryById = async (
  id: number
): Promise<{ id: number; content: string; date: string } | null> => {
  const db = await openDatabase();

  const row = await db.getFirstAsync(
    `SELECT id, note as content, date FROM journal_entries WHERE id = ?`,
    id
  );

  if (!row) return null;

  const entry = row as JournalEntryRow;
  return {
    id: entry.id,
    content: entry.content,
    date: entry.date,
  };
};

export const updateJournalEntry = async (
  id: number,
  content: string
): Promise<void> => {
  const db = await openDatabase();

  await db.runAsync(
    "UPDATE journal_entries SET note = ? WHERE id = ?",
    content,
    id
  );
};

export const deleteJournalEntry = async (id: number): Promise<void> => {
  const db = await openDatabase();

  await db.runAsync("DELETE FROM journal_entries WHERE id = ?", id);
};
